            ...
            for (int _x = xmin; _x < xmax; _x++) {
                ...
                if (insideTriangle(x, y, t.v)) {
                    ...
                    if (z_interpolated < depth_buf[index]) {
                        ...
                        Vector2f tex_coords_interpolated = alpha * t.tex_coords[0] / v[0].w() + beta * t.tex_coords[1] / v[1].w() + gamma * t.tex_coords[2] / v[2].w();
                        tex_coords_interpolated *= w_reciprocal;
                        tex_coords_interpolated[0] = std::clamp(tex_coords_interpolated[0], 0.0f, 1.0f);
                        tex_coords_interpolated[1] = std::clamp(tex_coords_interpolated[1], 0.0f, 1.0f);
                        ...
                        fragment_shader_payload payload( color_interpolated, normal_interpolated.normalized(), tex_coords_interpolated, diffuse_texture ? &*diffuse_texture : nullptr, normal_texture ? &*normal_texture : nullptr);
